...
Passing `to` is optional. When you do, it must be `user:<id>` — a WhatsApp user, and the agent's creator. An `agent:<id>`, a bare phone number or any other shape is rejected (HTTP 400, code `131009`); the library catches these before the request leaves. Take it from a recent inbound message and pass it back unchanged — never hardcode it.

## Text

```python
client.send_text("Hello", preview_url=True, reply_to="wamid.HBg...")
```
...
client.send_image(media_id=media_id, caption="our logo")

# or in one call
client.send_image(file="cat.jpg", caption="look at this")
client.send_image(file=open("cat.jpg", "rb"))
client.send_image(file=photo_bytes, mime_type="image/jpeg")
```
...
```python
client.send_document(file="invoice.pdf")                      # filename: invoice.pdf
client.send_document(media_id=mid, filename="invoice.pdf")    # set it yourself
```
...
```python
client.send_text("On it", reply_to=message.id)
...
```python
client.send_message(None, "text", {"body": "Hello", "preview_url": True})   # None: the known recipient
```
